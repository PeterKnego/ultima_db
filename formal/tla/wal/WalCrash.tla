...
(*     bump in Submit, M4 = TailTolerant, M5 = SyncData's metaDurable       *)
...
TailTolerant(sk) == sk = "CoalescedPrealloc" /\ MUTATION # "M4"
...
(*                                                                         *)
(* Implied by RecoverySound, so it is clean wherever RecoverySound is;      *)
(* modes/ConsistentPrealloc3.cfg carries it as the same-bound control.      *)
(***************************************************************************)
...
(*      maximal present-prefix of the on-disk log is replayed. Without this *)
(*      clause a "tolerant" scan that silently threw the whole log away     *)
(*      would satisfy the property -- which is the strict path's actual     *)
(*      behaviour (StrictScanErrLosesDurableAck below), so the distinction  *)
(*      is not hypothetical.                                                *)
(*                                                                         *)
