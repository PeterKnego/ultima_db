...
(*                                                                         *)
(* This matters for M5 specifically. The M5 failure that strands            *)
(* ALREADY-DURABLE, ALREADY-ACKED data behind an un-synced size change      *)
(* lives exactly in the unreached region: at MaxCommits = 2 the only frames *)
(* the frontier can strand are the first batch's, which were never acked.   *)
(* M5 does still bite at 2, so this was never a correctness hole -- but a   *)
(* mutation that bites only for the small reason, on the config someone     *)
(* actually runs, is a weaker calibration than one that bites for the real  *)
(* one. modes/ConsistentPrealloc3.cfg buys the real one for ~2 seconds.     *)
(***************************************************************************)
