...
fn multi_table_write_is_atomic() {
    ...
    let rtx = store.begin_read(None).unwrap();
    assert_eq!(
        rtx.open_table::<String>("users").unwrap().get(1),
...
fn two_overlapping_write_txs_to_different_tables() {
    ...
    wtx_a
        .open_table::<u32>("table_a")
        .unwrap()
        .insert(100u32)
        .unwrap();
    wtx_b
        .open_table::<u32>("table_b")
        .unwrap()
        .insert(200u32)
        .unwrap();
    ...
    let r1 = store.begin_read(Some(1)).unwrap();
    assert_eq!(
        r1.open_table::<u32>("table_a").unwrap().get(1),
        Some(&100u32)
    );
    assert!(matches!(
    ...
    let r2 = store.begin_read(Some(2)).unwrap();
    assert_eq!(
        r2.open_table::<u32>("table_a").unwrap().get(1),
        Some(&100u32)
    );
    assert_eq!(
        r2.open_table::<u32>("table_b").unwrap().get(1),
        Some(&200u32)
    );
}
...
fn empty_commit_produces_new_version_with_same_data() {
    ...
    let rtx = store.begin_read(None).unwrap();
    assert_eq!(
        rtx.open_table::<String>("t").unwrap().get(1),
...
fn old_read_snapshot_survives_many_writes() {
    ...
    assert_eq!(
        latest.open_table::<String>("t").unwrap().get(1),
        Some(&"version 10".to_string())
    );
}
...
fn index_works_across_snapshots() {
    ...
    assert!(
        t_v1.get_unique::<String>("by_email", &"alice@x.com".to_string())
            .unwrap()
            .is_some()
    );
    assert!(
        t_v1.get_unique::<String>("by_email", &"bob@x.com".to_string())
            .unwrap()
            .is_none()
    );
...
fn compound_index_on_multiple_fields() {
    ...
    // Index on (age, name)
    table
        .define_index("by_age_name", IndexKind::Unique, |u: &User| {
            (u.age, u.name.clone())
        })
        .unwrap();
...
fn multi_writer_disjoint_tables_both_commit() {
    ...
    assert_eq!(
        rtx.open_table::<String>("t1").unwrap().get(1),
        Some(&"a".to_string())
    );
    assert_eq!(
        rtx.open_table::<String>("t2").unwrap().get(1),
        Some(&"b".to_string())
    );
}
...
fn concurrent_disjoint_tables_both_commit() {
    ...
    assert_eq!(
        rtx.open_table::<String>("t_a").unwrap().get(1),
        Some(&"a".to_string())
    );
    assert_eq!(
        rtx.open_table::<String>("t_b").unwrap().get(1),
        Some(&"b".to_string())
    );
}
...
fn ssi_prevents_write_skew_via_table_scan() {
    ...
    assert!(
        ...
        "expected SerializationFailure on 'doctors', got {:?}",
        res
    );
...
fn si_allows_write_skew_table_scan() {
    ...
    wtx_b
        .commit()
        .expect("B commits in SI (write skew permitted)");
...
fn ssi_read_then_write_conflicts_on_concurrent_modify() {
    ...
    assert!(
        ...
        "expected SerializationFailure on 't', got {:?}",
        res
    );
}
...
fn ssi_disjoint_point_reads_dont_conflict() {
    ...
    wtx_a
        .commit()
        .expect("A commits — disjoint read/write sets");
