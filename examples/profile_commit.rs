...
fn main() {
    ...
    let bursts: Vec<Vec<Vec<Op>>> = (0..NUM_BURSTS)
        .map(|_| {
            (0..NUM_WRITERS)
                .map(|_| gen_ops(&mut rng, OPS_PER_WRITER))
                .collect()
        })
        .collect();
    ...
    println!(
        "phase 0 (acquire table locks):  {:>8.1} ms  ({:>5.1}%)",
        p0 as f64 / 1e6,
        100.0 * p0 as f64 / total_phase as f64
    );
    println!(
        "phase 1 (read + OCC validate):  {:>8.1} ms  ({:>5.1}%)",
        p1 as f64 / 1e6,
        100.0 * p1 as f64 / total_phase as f64
    );
    println!(
        "phase 2 (merge, lock-free):     {:>8.1} ms  ({:>5.1}%)",
        p2 as f64 / 1e6,
        100.0 * p2 as f64 / total_phase as f64
    );
    println!(
        "phase 3 (global write lock):    {:>8.1} ms  ({:>5.1}%)",
        p3 as f64 / 1e6,
        100.0 * p3 as f64 / total_phase as f64
    );
    println!(
        "sum of phases:                  {:>8.1} ms",
    ...
    println!(
        "total avg:    {:>7.1} µs",
        total_phase as f64 / 1000.0 / n as f64
    );
}
