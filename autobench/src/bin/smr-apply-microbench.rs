...
//! Fitness binary for the `smr-apply` task. One JSON object on stdout.
...
struct Args {
    ...
    /// Tolerance recorded into --write-baseline (commit-path noise: 5%).
...
fn main() {
    ...
    let metrics: BTreeMap<String, f64> = smr_bench::run(&smr_bench::Config::from_env());
