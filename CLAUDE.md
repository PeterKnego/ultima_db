...
- **`BTree<K, V>`** (`src/btree.rs`): Persistent B-tree (T=32 default, MAX_KEYS=63; T=8 behind the `fanout-t8` feature for write-dominated SMR deployments — see task52). Nodes use inline fixed-capacity storage (`FixedVec`), so a CoW node clone is one allocation. `insert`/`remove` return a *new* `BTree`; `Clone` is O(1) (Arc bump on root). Values stored as `Arc<V>` — no `V: Clone` bound needed. Re-exported as `ultima_db::BTree` (also the recommended backing structure for `CustomIndex` internals).
...
- **`Store`** (`src/store.rs`): Version history as `HashMap<u64, Arc<Snapshot>>`. `Store: Send + Sync + Clone` — share clones across threads. Constructed via `Store::new(StoreConfig)` (or `Store::default()` for defaults); build the `StoreConfig` with `StoreConfig::builder()....build()` (it is `#[non_exhaustive]`, task44). Provides `begin_read(Option<u64>)`, `begin_write(Option<u64>)`, `pin_version(Option<u64>)` (a `Send`-able `VersionPin` that keeps one version alive across `gc()` — the SMR snapshot-handoff primitive, task53), and `gc()` (O(evicted + pins) per run, not O(retained)). `StoreConfig` controls `num_snapshots_retained` (default 10, how many recent snapshots `gc()` keeps), `auto_snapshot_gc` (default true), `writer_mode` (`SingleWriter` default, or `MultiWriter` for concurrent writers with OCC), and `isolation_level` (`SnapshotIsolation` default, or `Serializable` for read-set tracking that prevents write skew under MultiWriter — see `docs/tasks/task21_serializable_isolation.md`).
- **`ReadTx`/`WriteTx`** (defined in `src/store.rs`, re-exported via `src/transaction.rs`): `ReadTx` holds `Arc<Snapshot>` — zero-copy reads. `WriteTx` lazily clones tables on first `open_table` (O(1) per table). `ReadTx` is `Send + Sync`; `WriteTx` is `Send + !Sync` (the `RefCell` write/read/DDL sets) — a transaction can be moved between threads but never used from two at once. The `PhantomData<*const ()>` `!Send` marker was removed in task55 after an audit found no thread affinity on the write path (`tests/send_bounds.rs` asserts the bounds). The recommended pattern is still to clone the `Store` and call `begin_write`/`begin_read` inside each thread; an open `WriteTx` holds the SingleWriter slot / its MultiWriter intents, and both `commit` and `Drop` block on the store write lock (`Drop` includes early returns and cancelled async tasks), so on an async runtime the whole open/use/commit-or-drop sequence belongs in `spawn_blocking`.
