...
- **`autobench/`**: autoresearch-style perf harness (`run-iter`, `journal-microbench`, `smr-apply-microbench`) with committed baselines (`make perf/check`) and frozen torture floors. See `docs/tasks/task35_autobench_perf_harness.md` and `autobench/CLAUDE.md`.
...
- Index DDL (`define_index`/`define_custom_index`) inside a MultiWriter transaction is not conflict-checked and is silently dropped if the commit takes the merge slow path — define indexes in their own transaction with no concurrent writers on the table (see task21 "v1 limitations").
