...
`Store` → `HashMap<u64, Arc<Snapshot>>` → `HashMap<String, Arc<dyn Any + Send + Sync>>` → `Table<R>` → `BTree<u64, R>` → `Arc<BTreeNode<u64, R>>`
...
- **Type erasure**: Tables are `Arc<dyn Any + Send + Sync>` in snapshots, `Box<dyn Any + Send + Sync>` in WriteTx's dirty map. Downcast at `open_table` time; `Error::TypeMismatch` if wrong type.
...
**MultiWriter OCC (table-level):** In `WriterMode::MultiWriter`, any two writers that touch the same table conflict, even if their modified keys are disjoint. The loser's `commit()` returns `Error::WriteConflict` — retry with a fresh `WriteTx`; the retry rebases onto the current latest snapshot. Non-overlapping tables (writer A writes `t1`, writer B writes `t2`) both commit, and the later commit rebases so the final snapshot contains both. See `examples/concurrent_writes.rs` for the retry pattern.
...
- `WriteTx::commit` rebases the new snapshot onto the current `latest_version` (not `self.base`). OCC (table-level) has already verified no concurrent commit touched a table we wrote to, so merging our dirty tables onto latest is safe and ensures concurrent non-conflicting commits don't lose each other's changes.
