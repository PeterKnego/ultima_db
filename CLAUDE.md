...
- **`BTree<K, V>`** (`src/btree.rs`): Persistent B-tree (T=64, MAX_KEYS=127). `insert`/`remove` return a *new* `BTree`; `Clone` is O(1) (Arc bump on root). Values stored as `Arc<V>` — no `V: Clone` bound needed. Re-exported as `ultima_db::BTree` (also the recommended backing structure for `CustomIndex` internals).
